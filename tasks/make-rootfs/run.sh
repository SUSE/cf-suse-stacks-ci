#!/bin/bash

set -o errexit
set -o nounset
set -o pipefail
set -o xtrace

SUFFIX="${STACKS_SUFFIX:-}"

src-ci/concourse/tasks/common/start-docker.sh

make -C src

versioned_stack_filename="out/opensusefs2${SUFFIX}-$(cat version/number).tar.gz"
mv src/opensusefs2.tar.gz "${versioned_stack_filename}"

versioned_receipt_filename="out/opensusefs2${SUFFIX}-$(cat version/number).receipt.txt"
echo "Rootfs SHASUM: $(sha1sum "$versioned_stack_filename" | awk '{print $1}')" > "$versioned_receipt_filename"
echo "" >> "$versioned_receipt_filename"
cat src/cflinuxfs2/opensusefs2_zypper.out >> "$versioned_receipt_filename"

# No need to diff the rootfs shasum, that will always be different
diff -u \
    <(tail -n +2 "receipt/opensusefs2${SUFFIX}-$(cat receipt/version).receipt.txt") \
    <(tail -n +2 "$versioned_receipt_filename") \
    > "out/opensusefs2${SUFFIX}-$(cat version/number).receipt.diff" \
    || true
